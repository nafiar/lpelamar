<footer class="page-footer" >
    <div class="container">
        <div class="row">
            <div class="col l6 s12">
                <h5 class="white-text"> <img class="valign" style="height:1.8rem" src="{{url('image/logo.png')}}">
                    Programming Laboratory
                </h5>
                <p class="grey-text text-lighten-4"></p>
            </div>
            <div class="col l4 offset-l1 s12">
                <h5 class="white-text">Links</h5>
                <ul>
                    <li><a class="grey-text text-lighten-3" href="http://if.its.ac.id">Informatics Department ITS</a></li>
                    <li><a class="grey-text text-lighten-3" href="#">Programming Laboratory</a></li>
                    <li><a class="grey-text text-lighten-3" href="#">LPortal</a></li>
                    <li><a class="grey-text text-lighten-3" href="#">LP Research Division</a></li>
                </ul>
            </div>
        </div>
    </div>
    <div class="footer-copyright">
        <div class="container white-text">
            © 2016 Developed by LP <i> Developer Team </i>, terms and rights reserved.
            <a class="grey-text text-lighten-4 right" href="http://lp.if.its.ac.id">Official Website</a>
        </div>
    </div>
</footer>
